...
const byte pinMotorCCW = 7;                      // Motor on izquierda
const byte pinValvulaC = 8;                      // Electrovalvula para agua fria
const byte pinValvulaH =12;                      // Electrovalvula para agua caliente
const byte pinBomba = 13;                        // Bomba de desague
const byte pinLedExprimir = 5;                   // Led tablero exprimir
const byte pinLedEnjuagar = 6;                   // Led tablero enjuagar
const byte pinLedLavar = 9;                      // Led tablero lavar
const byte pinLedRemojo = 10;                    // Led tablero remojo
const byte pinLedTapa = 11;                      // Led tablero seguro tapa
...
void updatePrincipal(void) {
    ...
    if ( selWork != selector ){                                                         //  DEBUG
        Serial.print("Selector: ");                                                     //  DEBUG
        Serial.print(selector);                                                         //  DEBUG
        Serial.print("   aux: ");                                                       //  DEBUG
        Serial.println(aux);                                                            //  DEBUG
    }                                                                                   //  DEBUG
    if ( alarm && (actual > e_principal) ) e_principal = actual + antirebote;
    if ( !isWork ){
        if ( alarmStatus == 1 ) Serial.println("Error Selector");                       //  DEBUG
...
void updateStatus(void) {
    ...
    if ( flag_PL && !pausa ){
        if ( !digitalRead(pinSeleccion) ){
            ...
            if ( est && (selWork != 7) ){
                ...
                Serial.print("Seleccion ");                                             //  DEBUG
                Serial.println(funcionC);                                               //  DEBUG
            }
            for (byte i = 0; i < 5; i++)
                v_fcopia[i] = v_funcion[i];
        }
    ...
    if ( now != aguaTop ){
        ...
        if( actual > e_reboteP ){
            ...
            Serial.print("Presostato: ");                                               //  DEBUG
            Serial.println(aguaTop);                                                    //  DEBUG
    ...
    if ( now != tapaLock ){
        ...
        if( actual > e_reboteLT ){
            ...
            Serial.print("TapaLock: ");                                                 //  DEBUG
            Serial.println(tapaLock);                                                   //  DEBUG
...
void resetFunciones(){
    ...
    for (byte i = 0; i < 5; i++)
        v_fcopia[i] = v_funcion[i];
    Serial.println("resetFunciones");                                                   //  DEBUG
}
...
void chacachaca(unsigned int c_max,bool rem,byte k,byte tipo){
    if (!flag_llenado){
        ...
        if ( !aguaDone ){
            alarmStatus = 1;
            Serial.println("Error por agua chch");                                      //  DEBUG
        }
        if ( c_ciclos < c_max ){
            /****************************************************************************************/
            ...
            * ccw OFF           ccw ON   ccw OFF                    ccw ON   ccw OFF                    ccw ON...  */
            if (rem==1){// movimiento de remojo
                ...
                if (c_ciclos == 0){
                    ...
                    e_led_chch = e_ch_cw;
                    c_ciclos++;
                    Serial.println("Esperar 4 segundo");                                //  DEBUG
                    Serial.println("Ini chch. remojo");                                 //  DEBUG
                }
                if (cw)
            ...
            if (rem == 0){// movimiento de lavado
                ...
                if (c_ciclos == 0){
                    ...
                    c_ciclos++;
                    Serial.println("Esperar 4 segundo");                                //  DEBUG
                    Serial.println("Ini chch. normal");                                 //  DEBUG
                }
                if (cw)
        ...
        /****************************************************************************************/
            if( actual > e_ch_cw ){
                ...
                c_ciclos++;
                Serial.print("c_ciclos: ");                                             //  DEBUG
                Serial.println(c_ciclos);                                               //  DEBUG
                Serial.print("tiempo cw:  ");                                           //  DEBUG
                Serial.print(actual - genesis);                                         //  DEBUG
                Serial.print("   --int--: ");                                           //  DEBUG
                Serial.println(INTERVALO_CW);                                           //  DEBUG
            }
            ...
            if( actual > e_ch_ccw ){
                ...
                Serial.print("c_ciclos: ");                                             //  DEBUG
                Serial.println(c_ciclos);                                               //  DEBUG
                Serial.print("tiempo ccw: ");                                           //  DEBUG
                Serial.print(actual - genesis);                                         //  DEBUG
                Serial.print("   --int--: ");                                           //  DEBUG
                Serial.println(INTERVALO_CCW);                                          //  DEBUG
            }
            if( actual > e_led_chch ){
                toggleOneLed(k+1);
                e_led_chch += INTERVALO_LED;
                Serial.print("tiempo led: ");                                           //  DEBUG
                Serial.print(actual - genesis);                                         //  DEBUG
                Serial.print("   --int--: ");                                           //  DEBUG
                Serial.println(INTERVALO_LED);                                          //  DEBUG
            ...
            if( actual > endWork ){ 
                ...
                ccw = true;
                chcchc = false;
                Serial.println("Done chch: ");                                          //  DEBUG
                Serial.print("v_fcopia: ");                                             //  DEBUG
                for (byte i = 0; i < 5; i++)                                            //  DEBUG
                    Serial.print(v_fcopia[i]);                                          //  DEBUG
...
void toggleCW(){
    ...
    if( digitalRead( pinMotorCW ) ){
        digitalWrite(pinMotorCW, LOW);
        Serial.println("MotorCW OFF");                                                  //  DEBUG
    }
    else{
        digitalWrite(pinMotorCCW, LOW);
        digitalWrite(pinMotorCW, HIGH);
        Serial.println("MotorCW ON");                                                   //  DEBUG
    }
}
...
void toggleCCW(){
    ...
    if( digitalRead( pinMotorCCW ) ){
        digitalWrite(pinMotorCCW, LOW);
        Serial.println("MotorCCW OFF");                                                 //  DEBUG
    }
    else{
        digitalWrite(pinMotorCW, LOW);
        digitalWrite(pinMotorCCW, HIGH);
        Serial.println("MotorCCW ON");                                                  //  DEBUG
    }
}
// lleado del tambor
void llenado(void) {
    ...
    if (!aguaTop){// presostato abierto - por llenar
        if ( chcchc ){// No deberia estar en este punto en pleno lavado 
            alarmStatus = 1;
            Serial.println("Error, ya debio haber llenado");                            //  DEBUG
        }
        if ( !f_calibrar ){
...
void exprimir(unsigned long tiempo,byte k){
    ...
    if ( c_exp < (exp_cicloIII+1) ){//131
        if ( !flag_bomba ){
            ...
            lockT = actual + t_tapa;
            Serial.println("Bomba On        TAPA y PRESOSTATO");                        //  DEBUG
        }
        if( actual > lockT ){
            if ( !tapaLock ){
                alarmStatus = 1;
                Serial.println("Error Tapa");                                           //  DEBUG
            }
            else digitalWrite(pinLedTapa,LOW);                  // eliminar
        }
        if (!aguaTop){// esperar que drene el agua
            if ( !flag_exp ){
                flag_exp = true;
                drenado = actual + 3000UL;
                e_ch_cw = actual + 3000UL;
            }
            if( actual > drenado ){
                if ( c_exp < exp_cicloI ){
                    ...
                    if ( c_exp < exp_cicloII ){
                        ...
                        if ( c_exp < exp_cicloIII ){
                            ...
                            e_ch_cw += INTERVALO_CW;
                            Serial.println("Fin toggle");                               //  DEBUG
                            Serial.print("actual: ");                                   //  DEBUG
                            Serial.print(actual - genesis);                             //  DEBUG
                            Serial.print("  e_e_cwF: ");                                //  DEBUG
                            Serial.println(e_e_cwF - genesis);                          //  DEBUG
                        }
                    }
                }
                ...
                if( actual > e_ch_cw ){
                    ...
                    e_ch_cw += INTERVALO_CW;
                    c_exp++;
                    Serial.print("c_exp: ");                                            //  DEBUG
                    Serial.println(c_exp);                                              //  DEBUG
            ...
            if( actual > e_bomba ){
                alarmStatus = 1;
                Serial.println("Error por presostato");                                 //  DEBUG
            }
        }
    }else{
        if (actual < e_e_cwF){
            if ( c_exp == (exp_cicloIII+1) ){
                digitalWrite(pinMotorCW, HIGH);
                c_exp++;
                Serial.println("Sostenido");                                            //  DEBUG
                Serial.print("actual: ");                                               //  DEBUG
                Serial.println(actual - genesis);                                       //  DEBUG
            }
            ...
            if( actual > endWork ){ 
                ...
                c_roc = 5;
                update_leds();
                Serial.println("Fin exprimir");                                         //  DEBUG
            }
        }
    }
}
...
void setup() {
    ...
    genesis = millis();
    if ( OFF ) Serial.println("OFF: ");                                                 //  DEBUG
    else Serial.println("ON: ");                                                        //  DEBUG
    Serial.print("Selector: ");                                                         //  DEBUG
    Serial.println(selector);                                                           //  DEBUG
...
void inicioPausa(){
    ...
    if ( !OFF ){
        ...
        if( actual > e_reboteIP ){
            e_reboteIP = actual + antirebote;
            Serial.print("Interrupcion ");                                              //  DEBUG
